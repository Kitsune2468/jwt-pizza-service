...
test('bad create franchise by user', async () => {
  franchiseName = randomName();
  franchiseRes = await request(app).post('/api/franchise').set('Authorization', `Bearer ${testUserAuthToken}`).send({ name: `${testUserAuthToken}`, admins: [{ email: `${testUser.email}` }] });
  expect(franchiseRes.status).toBe(403);
});

// Get admin franchise test
test('get user franchise', async () => {
  franchiseRes = await request(app).get(`/api/franchise/${testAdminID}`).set('Authorization', `Bearer ${testAdminAuthToken}`);
  expect(franchiseRes.status).toBe(200);
});
...
test('bad create store user', async () => {
  storeName = randomName();
  storeRes = await request(app).post(`/api/franchise/${testFranchiseID}/store`).set('Authorization', `Bearer ${testUserAuthToken}`).send({ franchiseID: `${testFranchiseID}`,name: `${storeName}`});
  expect(storeRes.status).toBe(403);
});

// Delete store tests
test('delete store', async () => {
  storeRes = await request(app).delete(`/api/franchise/${testFranchiseID}/store/${testStoreID}`).set('Authorization', `Bearer ${testAdminAuthToken}`);
  expect(storeRes.status).toBe(200);
});

test('bad delete store user', async () => {
  storeName = randomName();
  storeRes = await request(app).delete(`/api/franchise/${testFranchiseID}/store/${testStoreID}`).set('Authorization', `Bearer ${testUserAuthToken}`);
  expect(storeRes.status).toBe(403);
});
